...
VSCode 装上 Swift 官方插件后，发现 LSP 居然不工作，只有 VSCode 自带的高亮。一查才发现：**官方 Swift 插件只支持 SPM 项目（即有 `Package.swift` 的项目）**，而我手上的项目是 `.xcodeproj` + CocoaPods 的传统结构，官方插件压根不认。
...
而 `.xcodeproj` / `.xcworkspace`？不在支持列表里。所以我们需要一个**桥梁**，把 Xcode 项目的编译信息"翻译"成 SourceKit-LSP 能理解的格式。
...
- Swift 插件 = 语言大脑（补全、跳转、诊断）
- SweetPad = 工具手脚（构建、运行、索引桥接）
...
用 VSCode 打开你的项目根目录（注意是项目根目录，不是 `.xcodeproj` 文件），然后打开命令面板（`Cmd+Shift+P`），执行：

```
SweetPad: Generate Build Server Config (buildServer.json)
```

这会在项目根目录生成一个 `buildServer.json`。这个文件就是那个"桥梁"——它告诉 SourceKit-LSP："去读 Xcode 的构建日志，编译参数都在那儿。"
